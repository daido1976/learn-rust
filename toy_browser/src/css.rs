#[derive(Debug)]
pub struct Stylesheet {
    pub rules: Vec<Rule>,
}

#[derive(Debug, PartialEq)]
pub struct Rule {
    pub selectors: Vec<Selector>,
    pub declarations: Vec<Declaration>,
}

#[derive(Debug, PartialEq, Eq)]
pub enum Selector {
    Simple(SimpleSelector),
}

pub type Specificity = (usize, usize, usize);

impl Selector {
    // http://www.w3.org/TR/selectors/#specificity
    pub fn specificity(&self) -> Specificity {
        let Selector::Simple(ref simple) = *self;
        let a = simple.id.iter().count();
        let b = simple.class.len();
        let c = simple.tag_name.iter().count();
        (a, b, c)
    }
}

#[derive(Debug, PartialEq, Eq)]
pub struct SimpleSelector {
    pub tag_name: Option<String>,
    pub id: Option<String>,
    pub class: Vec<String>,
}

#[derive(Debug, PartialEq)]
pub struct Declaration {
    pub name: String,
    pub value: Value,
}

#[derive(Debug, Clone, PartialEq)]
pub enum Value {
    Keyword(String),
    Length(f32, Unit),
    ColorValue(Color),
}

impl Value {
    /// Return the size of a length in px, or zero for non-lengths.
    pub fn to_px(&self) -> f32 {
        match *self {
            Value::Length(f, Unit::Px) => f,
            _ => 0.0,
        }
    }
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub enum Unit {
    Px,
}

#[derive(Debug, Clone, PartialEq, Eq, Default)]
pub struct Color {
    pub r: u8,
    pub g: u8,
    pub b: u8,
}

impl Copy for Color {}

/// Parse a whole CSS stylesheet.
pub fn parse(source: String) -> Stylesheet {
    let mut parser = Parser::new(0, source);
    Stylesheet {
        rules: parser.parse_rules(),
    }
}

struct Parser {
    position: usize,
    input: String,
}

impl Parser {
    /// Creates a new `Parser`.
    fn new(position: usize, input: String) -> Self {
        Parser { position, input }
    }

    /// Parse a list of rule sets, separated by optional whitespace.
    fn parse_rules(&mut self) -> Vec<Rule> {
        let mut rules = Vec::new();
        loop {
            self.consume_whitespace();
            if self.eof() {
                break;
            }
            rules.push(self.parse_rule());
        }
        rules
    }

    /// Parse a rule set: `<selectors> { <declarations> }`.
    fn parse_rule(&mut self) -> Rule {
        Rule {
            selectors: self.parse_selectors(),
            declarations: self.parse_declarations(),
        }
    }

    /// Parse a comma-separated list of selectors.
    fn parse_selectors(&mut self) -> Vec<Selector> {
        let mut selectors = Vec::new();
        loop {
            selectors.push(Selector::Simple(self.parse_simple_selector()));
            self.consume_whitespace();
            match self.current_char() {
                ',' => {
                    self.consume_char();
                    self.consume_whitespace();
                }
                '{' => break,
                c => panic!("Unexpected character {} in selector list", c),
            }
        }
        // Return selectors with highest specificity first, for use in matching.
        selectors.sort_by_key(|s| std::cmp::Reverse(s.specificity()));
        selectors
    }

    /// Parse one simple selector, e.g.: `type#id.class1.class2.class3`
    fn parse_simple_selector(&mut self) -> SimpleSelector {
        let mut selector = SimpleSelector {
            tag_name: None,
            id: None,
            class: Vec::new(),
        };
        while !self.eof() {
            match self.current_char() {
                '#' => {
                    self.consume_char();
                    selector.id = Some(self.parse_identifier());
                }
                '.' => {
                    self.consume_char();
                    selector.class.push(self.parse_identifier());
                }
                '*' => {
                    // universal selector
                    self.consume_char();
                }
                c if valid_identifier_char(c) => {
                    selector.tag_name = Some(self.parse_identifier());
                }
                _ => break,
            }
        }
        selector
    }

    /// Parse a list of declarations enclosed in `{ ... }`.
    fn parse_declarations(&mut self) -> Vec<Declaration> {
        assert_eq!(self.consume_char(), '{');
        let mut declarations = Vec::new();
        loop {
            self.consume_whitespace();
            if self.current_char() == '}' {
                self.consume_char();
                break;
            }
            declarations.push(self.parse_declaration());
        }
        declarations
    }

    /// Parse one `<property>: <value>;` declaration.
    fn parse_declaration(&mut self) -> Declaration {
        let property_name = self.parse_identifier();
        self.consume_whitespace();
        assert_eq!(self.consume_char(), ':');
        self.consume_whitespace();
        let value = self.parse_value();
        self.consume_whitespace();
        assert_eq!(self.consume_char(), ';');

        Declaration {
            name: property_name,
            value,
        }
    }

    fn parse_value(&mut self) -> Value {
        match self.current_char() {
            '0'..='9' => self.parse_length(),
            '#' => self.parse_color(),
            _ => Value::Keyword(self.parse_identifier()),
        }
    }

    fn parse_length(&mut self) -> Value {
        Value::Length(self.parse_float(), self.parse_unit())
    }

    /// Parse a value of length as float.
    /// If cannot be parsed into type f32, returns the default value of `0.0`.
    fn parse_float(&mut self) -> f32 {
        let s = self.consume_while(|c| matches!(c, '0'..='9' | '.'));
        s.parse().unwrap_or_default()
    }

    fn parse_unit(&mut self) -> Unit {
        match &*self.parse_identifier().to_ascii_lowercase() {
            "px" => Unit::Px,
            _ => panic!("unrecognized unit"),
        }
    }

    fn parse_color(&mut self) -> Value {
        assert_eq!(self.consume_char(), '#');
        Value::ColorValue(Color {
            r: self.parse_hex_pair(),
            g: self.parse_hex_pair(),
            b: self.parse_hex_pair(),
        })
    }

    /// Parse two hexadecimal digits.
    fn parse_hex_pair(&mut self) -> u8 {
        let s = &self.input[self.position..self.position + 2];
        self.position += 2;
        u8::from_str_radix(s, 16).expect("should pass radix in the range from 2 to 36.")
    }

    /// Parse a property name or keyword.
    fn parse_identifier(&mut self) -> String {
        self.consume_while(valid_identifier_char)
    }

    /// Consume and discard zero or more whitespace characters.
    fn consume_whitespace(&mut self) {
        self.consume_while(char::is_whitespace);
    }

    /// Consume characters until `predicate` returns false.
    fn consume_while<F>(&mut self, predicate: F) -> String
    where
        F: Fn(char) -> bool,
    {
        let mut result = String::new();
        while !self.eof() && predicate(self.current_char()) {
            result.push(self.consume_char());
        }
        result
    }

    /// Return the current character, and advance self.position to the next character.
    fn consume_char(&mut self) -> char {
        let mut iter = self.input[self.position..].char_indices();
        let (_, current_char) = iter.next().expect("reach the eof");
        let (next_position, _) = iter.next().unwrap_or((1, ' '));
        self.position += next_position;
        current_char
    }

    // Read the current character without consuming it.
    fn current_char(&self) -> char {
        self.input.chars().nth(self.position).unwrap_or_default()
    }

    /// Return true if all input is consumed.
    fn eof(&self) -> bool {
        self.position >= self.input.len()
    }
}

fn valid_identifier_char(c: char) -> bool {
    matches!(c, 'a'..='z' | 'A'..='Z' | '0'..='9' | '-' | '_')
}

#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn test_parse_rules() {
        let mut parser = Parser::new(
            0,
            "h1 { margin: auto; } .class { padding: 10px; } ".to_string(),
        );
        let selectors_1 = vec![Selector::Simple(SimpleSelector {
            tag_name: Some("h1".to_string()),
            id: None,
            class: Vec::new(),
        })];
        let declarations_1 = vec![Declaration {
            name: "margin".to_string(),
            value: Value::Keyword("auto".to_string()),
        }];

        let selectors_2 = vec![Selector::Simple(SimpleSelector {
            tag_name: None,
            id: None,
            class: vec!["class".to_string()],
        })];
        let declarations_2 = vec![Declaration {
            name: "padding".to_string(),
            value: Value::Length(10.0, Unit::Px),
        }];
        let rules = vec![
            Rule {
                selectors: selectors_1,
                declarations: declarations_1,
            },
            Rule {
                selectors: selectors_2,
                declarations: declarations_2,
            },
        ];
        assert_eq!(parser.parse_rules(), rules);
    }

    #[test]
    fn test_parse_rule() {
        let mut parser = Parser::new(0, "h1 { margin: auto; }".to_string());
        let selectors = vec![Selector::Simple(SimpleSelector {
            tag_name: Some("h1".to_string()),
            id: None,
            class: Vec::new(),
        })];
        let declarations = vec![Declaration {
            name: "margin".to_string(),
            value: Value::Keyword("auto".to_string()),
        }];
        let rule = Rule {
            selectors,
            declarations,
        };
        assert_eq!(parser.parse_rule(), rule);
    }

    #[test]
    fn test_parse_selectors() {
        let mut parser = Parser::new(0, "h1, h2.class, h3#id {".to_string());
        let selectors = vec![
            Selector::Simple(SimpleSelector {
                tag_name: Some("h3".to_string()),
                id: Some("id".to_string()),
                class: Vec::new(),
            }),
            Selector::Simple(SimpleSelector {
                tag_name: Some("h2".to_string()),
                id: None,
                class: vec!["class".to_string()],
            }),
            Selector::Simple(SimpleSelector {
                tag_name: Some("h1".to_string()),
                id: None,
                class: Vec::new(),
            }),
        ];
        assert_eq!(parser.parse_selectors(), selectors);
    }

    #[test]
    fn test_parse_simple_selector() {
        let mut parser = Parser::new(0, "*".to_string());
        let selector = SimpleSelector {
            tag_name: None,
            id: None,
            class: Vec::new(),
        };
        assert_eq!(parser.parse_simple_selector(), selector);

        let mut parser = Parser::new(0, "h1".to_string());
        let selector = SimpleSelector {
            tag_name: Some("h1".to_string()),
            id: None,
            class: Vec::new(),
        };
        assert_eq!(parser.parse_simple_selector(), selector);

        let mut parser = Parser::new(0, ".class".to_string());
        let selector = SimpleSelector {
            tag_name: None,
            id: None,
            class: vec!["class".to_string()],
        };
        assert_eq!(parser.parse_simple_selector(), selector);

        let mut parser = Parser::new(0, "tag#id.class1.class2.class3".to_string());
        let selector = SimpleSelector {
            tag_name: Some("tag".to_string()),
            id: Some("id".to_string()),
            class: vec![
                "class1".to_string(),
                "class2".to_string(),
                "class3".to_string(),
            ],
        };
        assert_eq!(parser.parse_simple_selector(), selector);
    }

    #[test]
    fn test_parse_declarations() {
        let mut parser = Parser::new(
            0,
            "{ margin: auto; padding: 10px; color: #cc0000; }".to_string(),
        );

        assert_eq!(
            parser.parse_declarations(),
            vec![
                Declaration {
                    name: "margin".to_string(),
                    value: Value::Keyword("auto".to_string())
                },
                Declaration {
                    name: "padding".to_string(),
                    value: Value::Length(10.0, Unit::Px)
                },
                Declaration {
                    name: "color".to_string(),
                    value: Value::ColorValue(Color { r: 204, g: 0, b: 0 })
                }
            ]
        )
    }
}
